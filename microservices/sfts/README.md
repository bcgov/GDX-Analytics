...
- `"start_date"`: [OPTIONAL] (defaults to `today` as `YYYYMMDD` according to system timezone) Will be used to populate part of the resultant file name and may be used to determine query logic. It may be set to:
  ...
  - Conditionally, with `"slq_parse_key": "pmrp_date_range"` it can instead be set to one of `"min"` or `"max"`, where that is determined by the `MIN` or `MAX` value of the `date` column in `google.google_mybusiness_servicebc_derived`.
- `"end_date"`: [OPTIONAL]  (defaults to `today` as `YYYYMMDD` according to system timezone) Will be used to populate part of the resultant file name and may be used to determine query logic. It may be set to:
  - `"YYYYMMDD"` value where `YYYY` is a 4-digit year value, `MM` is a 2-digit month value, and `DD` is a two digit day value. For example: `"20200220"` would represent a start date of February 20th, 2020.
  - `"unsent"`, where it will interpret to `"max"`.
  - Conditionally, with `"slq_parse_key": "pmrp_date_range"` it can instead be set to one of `"min"` or `"max"`, where that is determined by the `MIN` or `MAX` value of the `date` column in `google.google_mybusiness_servicebc_derived`.
