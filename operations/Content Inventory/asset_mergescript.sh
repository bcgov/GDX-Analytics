...
if [ $# -eq 0 ] # if there are no input variables we are going to ask for filenames
     then

        read -p 'Asset Explore CSV: ' explore_var
        ...
        if  test -f $output_var; then #check if the provided file DOES NOT exists
            printf "ERROR: The file \"$output_var\" already exists\nAborting\n"
            exit 22 # abort the script
        fi
        # line below is the merge command based on the first column of Node ID
        # merge takes the Meta Data and merges on the Download
