BEGIN;
DROP TABLE IF EXISTS liquid_tables.seven_day_derived_page_views;
CREATE TABLE IF NOT EXISTS liquid_tables.seven_day_derived_page_views (

  page_view_id VARCHAR(255) ENCODE ZSTD NOT NULL,
  ...
  page_display_url VARCHAR(6000) ENCODE ZSTD,
  page_exclusion_filter VARCHAR(6000) ENCODE ZSTD,
  page_section VARCHAR(3000) ENCODE ZSTD,
  page_subsection VARCHAR(3000) ENCODE ZSTD,
  ...
  -- dcs id
  dcs_id VARCHAR(255) ENCODE ZSTD,

  CONSTRAINT page_view_id_pk PRIMARY KEY(page_view_id)
)

-- node_id is the joining key for cmslite_themes

DISTSTYLE KEY
DISTKEY (node_id)
SORTKEY (node_id);

INSERT INTO liquid_tables.seven_day_derived_page_views (
    SELECT *
    FROM derived.page_views
    WHERE page_view_start_time BETWEEN DATEADD(day,-7,CURRENT_DATE) AND CURRENT_DATE
);

ALTER TABLE liquid_tables.seven_day_derived_page_views OWNER TO microservice;
GRANT SELECT ON liquid_tables.seven_day_derived_page_views TO "looker";
